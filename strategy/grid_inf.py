...
# todo 没有查询历史账户总金额的接口，需要补一个每天查询总金额的定时任务来计算收益总k线
# think 假如定时任务中断了 新的上下网是否要最新更新还是根据上次的继续来，我认为还根据上次的继续就好因为保持连贯仓位才不会乱
    ...
    def strategy(self):
        ...
        try:
            ...
            for instId, strategy_config in strategy_class_config.items():
                ...
                # 判断当前是否有仓位
                pos = False
                # todo 这里可以后续想想如果说一直平空平的仓都没了还没碰到上布林线的情况，是合理存在的还是参数设置的不好，如果合理要处理下，仓都没了清仓重新打仓位
